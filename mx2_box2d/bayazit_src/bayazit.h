...
namespace p2bayazit
{
	...
	static wxArray<wxArray<p2bayazit::Point>> BayazitConvexor(wxArray<p2bayazit::Point> inArr){
		...
		int siz=result.size(); //sinon emscrptem veut pas
	 	wxArray<wxArray<p2bayazit::Point>> returnArr (siz);
	 	returnArr.retain();
		...
		copy(result.begin(),result.end(),resultarr);
	 	...
	 	for (int pi=0;pi<result.size();pi++) {
				...
				wxArray<p2bayazit::Point> tempArr (sizbis);
				tempArr.retain();
